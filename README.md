...
There have been many [requests to add additional support for generic programming](https://golang.org/wiki/ExperienceReports#generics) in Go.
	
There has been extensive discussion on [the issue tracker](https://golang.org/issue/15292) and on [a living document](https://docs.google.com/document/d/1vrAy9gMpMoS3uaVphB32uVXX4pi-HnNjkMEgyAHX4N4/view).
	
There have been several proposals for adding type parameters, which can be found by looking through the links above.
Many of the ideas presented here have appeared before. The main new features described here are the syntax and the examination of like clauses.
	
This draft design suggests extending the Go language to add a form of parametric polymorphism, where the type parameters are bounded not by a subtyping relationship but by explicitly defined structural constraints expressed by like clauses.
	
Among other languages that support parametric polymorphism this design is perhaps most similar to CLU or Ada, although the syntax is completely different. Like clauses somewhat resemble Java's generics type constraints.
	
This design does not support template metaprogramming or any other form of compile-time programming.
	
As the term _generic_ is widely used in the Go community, we will see it below as a shorthand to mean a function or type that takes type parameters.
	
Don’t confuse the term generic as used in this design with the same term in other languages like C++, C#, Java, or Rust; they have similarities but are not always the same.
...
Generic code is code that is written using types that will be specified later. Each unspecified type is called a _type parameter_. When the code is used the type parameter is set to a _type argument_. Here is a function that prints out each element of a slice, where the element type of the slice, here called `T`, is unknown. This is a trivial example of the kind of function we want to permit in order to support generic programming.
...
As you can see, the first decision to make is: how should the type parameter `T` be declared? In a language like Go, we expect every identifier to be declared in some way. Here we make a design decision: type parameters are similar to
ordinary non-type function parameters, and as such should be listed along with other parameters.

However, type parameters are not the same as non-type parameters, so although they appear in the parameters we want to distinguish them. That leads to our next design decision: in stead of having an additional argument list with only type parameters, we include the type parameters in the normal argument list, but the type declaration is preceded by the type keyword to distinguish them from normal parameters. Type parameters must be specified before the regular parameters.
...
This says that within the function `Print` the identifier `T` is a type parameter, a type that is currently unknown but that will be known when the function is called. Since `Print` has a type parameter, when we call it we must pass a type argument.	
Type arguments are passed together with the normal arguments. At the call site, the `type` keyword is used to distinguish type arguments from normal arguments.
	
	
```Go
Print(int type, []int{1, 2, 3})	
```
...
func Stringify(T type, s []T) (ret []string) {
  for _, v := range s {
...
Naturally, the same issue arises in other languages that support generic programming. 

In C++, for example, a generic function (in C++ terms, a function template) can call any method on a value of generic type. 
That is, in the C++ approach, calling `v.String()` is fine. If the function is called with a type that does not have a `String`
method, the error is reported at the point of the function call.
	
These errors can be lengthy, as there may be several layers of generic function calls before the error occurs, 
all of which must be reported for complete clarity.	
	
The C++ approach would be a poor choice for Go. One reason is the style of the language. 
In Go we don’t refer to names, such as, in this case, `String`, and hope that they exist.	
...
In general, all generic code must describe what the capabilities are that type arguments need to implement.
In this case, the  is pretty obvious: the type has to have a `String() string` method.
In other cases it may be much less obvious.
We don’t want to derive the contract from whatever `Stringify` happens to do.
If we did, a minor change to `Stringify` might change the required capabilities of type arguments.
That would mean that a minor change could cause code far away, that calls the function, to unexpectedly break.
It’s fine for `Stringify` to deliberately change its requirements, and force users to change.
What we want to avoid is `Stringify` changing its requirements accidentally.
This is an important rule that we believe should apply to any attempt to define generic programming in Go: 
there should be an explicit description of the requirements of the generic code.
These requirements are expressed using like clauses and type templates.
...
```Go
func Stringify(T type like interface {String() string}, s []T) (ret []string) {
  for _, v := range s {
...
Or alternatively, using type templates:

```Go
type Stringer like interface {
  String() string
}

func Stringify(T type like Stringer, s []T) (ret []string) {
  for _, v := range s {
	   ret = append(ret, v.String())
	 }
	 return ret
...
A like clause is introduced with a new keyword `like`. In generic functions, the `like` keyword follows the `type` keyword, 
in type templates, it follows the type name. The definition of a typpe template looks like the definition of a normal type,
except that the `like` keyword is placed in between the type name and it's definition.
...
First, like clauses are used to validate a set of type arguments.
As shown above, when a function with type parameters is called, it will be called with a set of type arguments.
When the compiler sees the function call, it will use the like clause to validate the type arguments.
If the type arguments are invalid, the compiler will report a type error: the call is using types that 
the function’s like clauses do not permit.
		
To validate the type arguments, each of the parameter types is replaced with the corresponding type argument 
(there must be exactly as many type arguments as there are parameter types; contracts may not be variadic).
	
Then, for every corresponding type filled in, it is then checked whether or not it has the capabilities 
expressed by it's like clause. If the type checking succeeds, the type arguments are valid.
		
In the example of the `stringer` contract seen earlier, we can see that the type argument used for `T` must implement an interface that has a `String` method (it can be be a struct with a `String` field of function type).
	
The `String` method must not take any arguments, and it must return a value of a type that is assignable to `string`. 
(As it happens, the only type assignable to `string` is, in fact, `string`.)	
If any of those statements about the type argument are not true, the like clause will fail when it is type checked.
	
	
### The party of the second part
		
A like clquse is not used only at the call site.	
It is also used to describe what the function using the like clause or type template, the function with type parameters, 
is permitted to do with those type parameters.
	
For a type parameters that does not use a like clause, such as the `Print` example shown earlier, 
the function is only permitted to use those type parameters in ways that any type may be used in Go.
...
parameter, or a value of the type parameter, the like clause must permit it.
	
Basically, if a concrete type is listed in the like clause, then the actual function 
is permitted to use the type in the same way as the concrete type would be used. 
In case the like clause mentions several types (more on that later), the actual 
function may use the intersection of the capabilities of the different concrete types
thus mentioned.
	
This is described in more detail later.	
For now, look at the `stringer` like clause example above.	
The like clause in the argument list given a value of type `T`, the function `Stringify` is permitted to	
call a method of type `String`, passing no arguments, to get a value of type `string`.

That is, naturally, exactly the operation that the `Stringify` function needs.
	
	
### Using a type template
		
We’ve seen how the `Stringer` type template can be used to verify that a type argument is suitable 
for the `Stringify` function, and we’ve seen how the like clause permits the `Stringify` function to call the `String`
method that it needs.
	
The final step is showing how the `Stringify` function uses the `Stringer` type template.

This is done by naming the type template in the like clause after each type parameter.	
...
The type parameters are followed by an optional like clause which may refer to concrete types or type templates.
Each type parameter may have a different like clause, but if the like clauase is the same, then the `T,V like type X` or
`T,V like ...` may be used to specify that.  When validating the like clauses, the type parameters, 
are checked in the order in which they appear in the function definition.
	
	
### Like clause and type template syntactic details
	
	
Before we continue, let’s cover a few details of the like clause and type template syntax.

A like clause is of the form `like X` or , in case of multiple types `like (X, Y,...)`

These like clauses mean that for the type parameter or in type template specified by them,
that any one of X, Y, ... can be substituted in place of T. In effect, the type parameter supports 
the intersection of the operations defined on X, Y, ...

The types mentioned in the like clause are called the "concete types".

The concrete types speficy the cababilities of the generic type parameter or type template as follows:

* If the concrete type X is a primitive type such as int, any type derived from int can be substituted.
* If the concrete type X is an interface type, any type implementing X can be substituted.
* If the concrete type X is a struct type, any struct type implementing all the public methods methods of X, 
  which also has all public members of X can be substituted.
* If X is an array type, map type, or channel type, then any array/map/channel type derived from X can 
  be substituted.

#### Type templates

A type template is in essence a type that defines a shorthand for a like clause. 
It is a generic type that has the capabilities specified by it's own like clause.
Inside the definition of a type template, like clauses may be used on any type to specify it is a type paramer.

```
type Node like(type Node) struct {
  NextNode * Node
}

type LinkedList like(type Node) struct {
  FirstNode * Node
}
```

#### Like causes with primitive types
	
Like clauses may contain primitive types.  

For example, this simple type template says that a value specified with a like clause Integer may be used as the primitive type int, byte, or rune would be.

```Go
type Integer like (byte, int, rune, int64, uint64, int32, uint32, int16, uint16, uint8, int8)
 
func FormatUnsigned(T type like Integer, v T) string {	
...
The like keyword may appear for function arguments, in type definitions, and in bodies of type definitions of type templates.

XXX How to make this Go1 compatible???

#### Exported type templates
		
Like other top level declarations, a type template is exported if its name starts with an upper-case letter.
An exported type template may be used by functions, types, or contracts in other packages.	
...
func Print2(type T1, type T2, s1 []T1, s2 []T2) { ... }
...
In `Print2Same` `s1` and `s2` must be slices of the same element type.	
Functions may have multiple type parameters, each with their own like clause.
	
```Go
	


type SetString like interface {
  Set(string)
}
...
### Parameterized types through type templates
...
```Go
type Vector like(type Element) []Element
	
```
	
A type’s parameters are just like a function’s type parameters. Within the type definition, the type parameters may be used 
like any other type.
...
type List like(type Element) struct {	
	next *List(Element)
	val  Element
}
...
// This is INVALID.
type P like(type Element1, type Element2) struct {
...
type PrintStringer like (Stringer, Printer)
```
...
func Index(type T like canEqual, s []T, e T) int {	
	for i, v := range s {	
...
check whether `EqualInt` satisfies the like clause that specifies `comparable.Equaler` as the concrete type.
	
We replace `T` in the body of `comparable.Equaler` with `EqualInt`, and
see whether the type T then implements the resulting interface.
`EqualInt` has a method `Equal` that accepts a parameter of type `EqualInt`, so all is well, and the compilation succeeds.
